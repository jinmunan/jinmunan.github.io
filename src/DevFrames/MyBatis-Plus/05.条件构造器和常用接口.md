...
![image-20230410144427542](./assets/image-20230410144427542.png)
...
查询用户名包含 a，年龄在 40 到 50 之间，邮箱信息不为 null 的信息
...
void selectList() {
    ...
    queryWrapper.isNotNull("email");//不为 null
...
@Test
void update02() {
    //查询用户名包含 a 并且年龄大于 20 或邮箱为 null 的用户信息修改
    //lambda 的表达式优先执行，i 表示条件构造器
    QueryWrapper<User> wrapper = new QueryWrapper<User>();
     wrapper.gt("age",20)
                .like("user_name","a")
                .or()
...
```java
/**
* QueryWrapper 只能是查询，修改还需要实体类
* UpdateWrapper 可以充当实体类
*/
@Test
void update03() {
    //查询用户名包含 a 并且年龄大于 20 或邮箱为 null 的用户信息修改
