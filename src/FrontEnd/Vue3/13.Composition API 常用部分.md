...
export default {
  ...
  /* 在 Vue3 中依然可以使用 data 和 methods 配置，但建议使用其新语法实现 */
  // data () {
  //   return {
  //     count: 0
  //   }
  ...
  <script>
    ...
    */
    const proxyUser = new Proxy(user, {
      ...
      set(target, prop, val) {
        console.log('劫持 set()', prop, val)
        return Reflect.set(target, prop, val); // (2)
      },

      deleteProperty (target, prop) {
        console.log('劫持 delete 属性', prop)
        return Reflect.deleteProperty(target, prop)
      }
    });
    // 读取属性值
...
- setup 执行的时机
  ...
  - this 是 undefined, 不能通过this来访问data/computed/methods / props
...
export default {
  ...
  setup () {
    ...
    // 使用 ref 处理对象  ==> 对象会被自动 reactive 为 proxy 对象
    const m3 = ref({a1: 2, a2: {a3: 'abc'}})
    console.log(m1, m2, m3)
    console.log(m3.value.a2) // 也是一个 proxy 对象
    ...
    function update() {
      ...
      m3.value.a2.a3 += '==' // reactive 对对象进行了深度数据劫持
  ...
  setup () {
    ...
    */
    watch(user, () => {
      ...
      immediate: true,  // 是否初始化立即执行一次，默认是 false
      deep: true, // 是否是深度监视，默认是 false
    })
...
![image-20230418222800897](./assets/image-20230418222800897.png)

### vue3 的生命周期

![image-20230327085318826](./assets/image-20230327085318826.png)
